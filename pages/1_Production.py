"""
SAP Infrastructure Dashboard - Production Environment
"""
...
st.set_page_config(
    page_title="PROD - Dashboard EPMAPS",
    page_icon="🚀",
    layout="wide",
...
def main_dashboard():
    ...
    create_env_switcher_header("Production")
    ...
    for i in range(0, len(server_groups), 2):
        ...
        with col1:
            if i < len(server_groups):
                create_group_from_config(server_groups[i], "prod")
        
        with col2:
            if i + 1 < len(server_groups):
                create_group_from_config(server_groups[i + 1], "prod")
